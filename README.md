...
Lệnh này gọi Tavily để lấy nguồn và OpenAI Responses API để viết câu trả lời single-agent có citation.
...
Lệnh này chạy workflow `Supervisor -> Researcher -> Analyst -> Writer` và in `ResearchState` cuối cùng, bao gồm route history, trace events, sources và final answer.

### 6. Chạy benchmark report
...
1. OpenAI LLM client qua Responses API.
2. Tavily search client chuẩn hóa về `SourceDocument`.
3. Supervisor routing deterministic với `max_iterations`.
4. Researcher, Analyst, Writer và optional Critic.
5. LangGraph workflow cho multi-agent orchestration.
6. Local trace events trong `ResearchState.trace`.
7. Benchmark command xuất `reports/benchmark_report.md`.
